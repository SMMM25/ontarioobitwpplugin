    ...
    <div class="ontario-obituary-detail-footer">
        ...
        <div class="ontario-obituary-assistant-actions">
            <a href="<?php echo esc_url(add_query_arg(array('source' => 'ontario', 'obituary_id' => $obituary->id), site_url('/obituary-assistant/'))); ?>" class="button ontario-obituary-assistant-button">
                <?php _e('Open in Obituary Assistant', 'ontario-obituaries'); ?>
            </a>
        </div>
        <?php endif; ?>
        ...
        <div class="ontario-obituary-social-sharing">
            ...
            <a href="#" class="ontario-obituary-share-facebook" data-id="<?php echo esc_attr($obituary->id); ?>" data-name="<?php echo esc_attr($obituary->name); ?>">
                <span class="dashicons dashicons-facebook"></span> <?php _e('Share on Facebook', 'ontario-obituaries'); ?>
            </a>
            <?php endif; ?>
            
            <a href="mailto:?subject=<?php echo esc_attr(sprintf(__('Obituary for %s', 'ontario-obituaries'), $obituary->name)); ?>&body=<?php echo esc_attr(sprintf(__('I thought you might want to see this obituary for %s: %s', 'ontario-obituaries'), $obituary->name, esc_url(add_query_arg(array('obituary_id' => $obituary->id), site_url())))); ?>" class="ontario-obituary-share-email">
                <span class="dashicons dashicons-email"></span> <?php _e('Share via Email', 'ontario-obituaries'); ?>
            </a>
        </div>
    </div>
</div>
