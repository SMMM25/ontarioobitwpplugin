<?php
/**
 * Shared Groq API Rate Limiter (v5.0.12 — QC-R12 hardening)
 *
 ...
 *
 * Architecture (v5.0.12 rewrite — addresses QC-R12 review):
 *
 ...
 *
 *   CAS STRATEGY (QC-R12-2): Uses SELECT … FOR UPDATE row-level lock
 ...
 *
 *   CACHE EFFICIENCY (QC-R12-4): Tiered cache invalidation:
 *     - After successful CAS write: invalidate per-option key ONLY (cheap).
 *     - After CAS failure (retry path): invalidate full triad.
 *     - After add_option(): invalidate per-option + notoptions + alloptions.
 *     QC-R12 improvement: The FOR UPDATE approach eliminates most CAS
 *     failures (concurrent readers block, not fail), so the full-triad
 *     invalidation path is rarely triggered. This reduces alloptions/
 *     notoptions churn from ~3 deletes on every contention to ~1 delete
 *     on the happy path.
 *
...
class Ontario_Obituaries_Groq_Rate_Limiter {
    ...
    private function cas_update( $new_json, $old_version ) {
        ...
        // Attempt transactional FOR UPDATE lock.
        ...
        // Row doesn't exist — another process deleted it. Rollback and fail.
        if ( null === $locked_row ) {
            $wpdb->query( 'ROLLBACK' );
            return 0;
        }

        // Decode and compare version in PHP — immune to key ordering.
